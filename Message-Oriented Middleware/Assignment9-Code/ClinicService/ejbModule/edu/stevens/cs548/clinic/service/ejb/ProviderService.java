	...
	public long addTreatment(TreatmentDto tdto) throws ProviderServiceExn {
		...
		try {
			...
			if (tdto.getDrugTreatment() != null) {
				Treatment dt = tf.createDrugTreatment(tdto.getDiagnosis(), tdto.getDrugTreatment().getDrug(), tdto.getDrugTreatment().getDosage());
				dt.setPatient(patient);
				provider.addTreatment(patient, dt);
				tid = dt.getId();
				tdto.setId(tid);
				
				//TreatmentDto treatment;
				ObjectMessage message = session.createObjectMessage();
				message.setObject(tdto);
				message.setStringProperty("treatmentType", "Drug");
				producer.send(message);
			} else if (tdto.getRadiology() != null) {
				Treatment rt = tf.createRadiologyTreatment(tdto.getDiagnosis(), tdto.getRadiology().getDate());
				rt.setPatient(patient);
				provider.addTreatment(patient, rt);
				tid = rt.getId();
				tdto.setId(tid);
				...
				Treatment st = tf.createSurgeryTreatment(tdto.getDiagnosis(), tdto.getSurgery().getDate());
				st.setPatient(patient);
				provider.addTreatment(patient, st);
				tid = st.getId();
				tdto.setId(tid);
